defmodule Appsignal.Probes do
  ...
  def handle_info(:run_probes, {probes, states}) do
    states = if Appsignal.Config.minutely_probes_enabled?() do
      stream = Task.async_stream(probes, fn {name, probe} ->
      ...
      end, ordered: false, timeout: 30000, on_timeout: :kill_task)
  ...
  defp call_probe(probe, state) when is_function(probe, 0) do
    probe.()
    nil
  end
